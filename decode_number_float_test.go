...
func TestDecoderFloat64OrNull(t *testing.T) {
	testCases := []struct {
		name           string
		json           string
		expectedResult float64
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if testCase.isNull {
				...
				assert.Equal(t, math.Round(testCase.expectedResult*1000000), math.Round(val*1000000), fmt.Sprintf("v must be equal to %f", testCase.expectedResult))
			}
		})
	}
	t.Run("decoder-api-invalid-json2", func(t *testing.T) {
		var dec = NewDecoder(strings.NewReader(``))
		_, _, err := dec.Float64OrNull()
		assert.NotNil(t, err, "Err must not be nil")
		assert.IsType(t, InvalidJSONError(""), err, "err should be of type InvalidJSONError")
	})
}
...
func TestDecoderFloat32OrNull(t *testing.T) {
	testCases := []struct {
		name           string
		json           string
		expectedResult float32
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if testCase.isNull {
				...
				assert.Equal(
					t,
					math.Round(float64(testCase.expectedResult*1000000)), math.Round(float64(val*1000000)),
					fmt.Sprintf("val must be equal to %f", testCase.expectedResult),
				)
			}
		})
	}
